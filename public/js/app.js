import { isFirebaseConfigured, auth } from "./firebase-init.js?v=1788800213033";
import { escapeHtml } from "./html-safety.js?v=1788800213033";
import { friendlyAuthError } from "./auth-errors.js?v=1788800213033";
import { getEffectiveTheme, toggleTheme } from "./theme.js?v=1788800213033";
import {
  watchAuthState,
  signInWithPassword,
  signOutUser,
...
import {
  ...
  isFullyLoaded,
  getClientRecord,
} from "./data.js?v=1788800213033";
import { renderCalendar } from "./calendar-view.js?v=1788800213033";
import { renderList } from "./list-view.js?v=1788800213033";
import { describeRecurrence, describeRecurrenceHistory, getLastDueOccurrence, toISODate } from "./recurrence.js?v=1788800213033";
import { colorFor, tintFor } from "./colors.js?v=1788800213033";
import { githubRepoSlug } from "./firebase-config.js?v=1788800213033";
import {
  DOC_TYPES,
  docTypeLabel,
  ...
  deleteDocument,
} from "./documents.js?v=1788800213033";
import { createClientInvite, subscribeToInviteStatus } from "./invites.js?v=1788800213033";
import { subscribeToOwnCompliance } from "./client-compliance.js?v=1788800213033";
import { subscribeToLetters, sendLetter, signLetter, deleteLetter, getLetterDownloadURL } from "./letters.js?v=1788800213033";
import {
  stampSignature,
  stampFields,
...
function computeAwaitingSignatureItems() {
  const rows = [];
  for (const letter of latestState.letters.values()) {
    const client = latestState.clients.get(letter.clientId);
    if (!client || client.archived) continue;
    if (letter.status === "sent") rows.push({ client, letter });
  }
  rows.sort((a, b) => (b.letter.sentAt?.toMillis() || 0) - (a.letter.sentAt?.toMillis() || 0));
  return rows;
}
...
function renderOverviewView() {
  ...
  const awaitingSignature = computeAwaitingSignatureItems();

  els.viewContainer.innerHTML = `
    <div class="stat-grid">
      <button type="button" class="stat-card stat-card-link" data-action="goto-clients">
      ...
      <button type="button" class="stat-card stat-card-link" data-action="goto-overdue">
        <div class="stat-card-header">
      ...
      <button type="button" class="stat-card stat-card-link" data-action="goto-signatures">
        <div class="stat-card-header">
        ...
        <div class="${awaitingSignature.length ? "stat-card-value" : "stat-card-value-muted"}">${awaitingSignature.length}</div>
      </button>
    </div>
  ...
  els.viewContainer
    .querySelector('[data-action="goto-clients"]')
    .addEventListener("click", () => setActiveView("clienthub"));
  els.viewContainer
    .querySelector('[data-action="goto-overdue"]')
    .addEventListener("click", () => setActiveView("list"));
  els.viewContainer
    .querySelector('[data-action="goto-signatures"]')
    .addEventListener("click", () => {
      // Only one client waiting makes it unambiguous which one to jump straight into; with
      // several, land on the Client Hub list instead, where each client's own attention badge
      // (see renderClientHubStaffView) already shows exactly who still needs a signature.
      if (awaitingSignature.length === 1) {
        clientHubExpandedId = awaitingSignature[0].client.id;
        clientHubActiveTab = "letters";
      }
      setActiveView("clienthub");
    });
  ...
  if (archivedCount > 0) {
    els.clientList.innerHTML += `
      <label class="show-archived-toggle">
        <span class="ios-toggle"><input type="checkbox" id="show-archived-checkbox" ${viewState.showArchived ? "checked" : ""} /></span>
        Show ${archivedCount} archived client${archivedCount === 1 ? "" : "s"}
      </label>`;
